...
def list_themes():
    ...
    target_lang = (request.args.get("target_lang") or "en").strip()
    with sqlite3.connect(get_db_path(target_lang)) as conn:
        cur = conn.cursor()
        cur.execute("SELECT id, name, target_lang, native_lang, cefr_default, created_at FROM themes ORDER BY id DESC")
        rows = cur.fetchall()
        out = [{"id": r[0], "name": r[1], "target_lang": r[2], "native_lang": r[3],
                "cefr_default": r[4], "created_at": r[5]} for r in rows]
        return jsonify(out)
...
def create_theme():
    ...
    pack = generate_theme_words(name, target_lang)
