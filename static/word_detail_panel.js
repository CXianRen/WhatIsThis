...
async function loadWordDetails(word,
  ...
  try {
    ...
    if (rightPanelTitle) {
      ...
      rightPanelTitle.textContent = `${data.word}`;
    }
    ...
    if (definitionText) definitionText.textContent = '加载失败';
    if (imagesGrid) imagesGrid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; color: #666;">图片加载失败</div>';
  }
}
