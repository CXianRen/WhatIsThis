// ========== Book Detail Component (ESM) ==========
import { LanguageUtils, LevelUtils, DateUtils, DOMUtils } from './utils.js';
...
function render(container) {
  ...
  const languageBadges = book.languages.map(lang => {
    const langInfo = LanguageUtils.getLanguageInfo(lang);
  ...
  // Level info
  const levelInfo = LevelUtils.getLevelInfo(book.level);
  ...
  const lastReadText = book.lastRead
    ? DateUtils.formatRelativeDate(book.lastRead)
    : 'Never read';

  const formattedDate = book.lastRead
    ? DateUtils.formatDate(book.lastRead)
    : null;
...
// Action functions that can be called from other modules
